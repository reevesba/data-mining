...
def main():
    ...
    ### using one splitting parameter, no weight ###
    ...
    total_cost = cost(COSTS, conf_matrix)
    f.write(f"Total Cost: ${total_cost:,.2f}\n\n")

    ### using one splitting parameter, weighted ###
    f.write("Two Splitting Parameters (with cost)\n")

    n_split_nodes = 2
    max_leaf_nodes = n_split_nodes + 1

    my_tree = tree.DecisionTreeClassifier(max_leaf_nodes=max_leaf_nodes, class_weight={0:15, 1:85})
    my_tree.fit(X_train, y_train)
    plot_tree(my_tree, feature_names, class_names, "1_split_c")

    y_pred = my_tree.predict(X_test)
    a, b, c, d = m.confusion_matrix(y_test, y_pred).ravel()
    conf_matrix = [a, b, c, d]
    plot_matrix(my_tree, X_test, y_test, class_names, "1_split_c")
    ...
    ### using two splitting parameters, no weight ###
    ...
    ### using two splitting parameters, weighted ###
    f.write("Two Splitting Parameters (with cost)\n")
    ...
    my_tree = tree.DecisionTreeClassifier(max_leaf_nodes=max_leaf_nodes, class_weight={0:15, 1:85})
    my_tree.fit(X_train, y_train)
    plot_tree(my_tree, feature_names, class_names, "2_split_c")

    y_pred = my_tree.predict(X_test)
    a, b, c, d = m.confusion_matrix(y_test, y_pred).ravel()
    conf_matrix = [a, b, c, d]
    plot_matrix(my_tree, X_test, y_test, class_names, "2_split_c")
